...
class CameraClient internal constructor(builder: Builder) : IPreviewDataCallBack {
    ...
    override fun onPreviewData(data: ByteArray?, format: IPreviewDataCallBack.DataFormat) {
        Log.v(TAG, "onPreviewData ${data?.size}")
        data?.let {
    ...
    /**
     * Add audio raw data call back
     *
     * @param callBack camera preview data call back, see [IAudioRawDataCallBack]
     */
    fun addAudioRawDataCallBack(callBack: IAudioRawDataCallBack) {
        (mAudioProcess as? AACEncodeProcessor)?.setPcmCallback(callBack)
    }
